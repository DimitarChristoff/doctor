...
var builder = prime({
	...
	buildDocs: function(markdown){
		...
		this.on('html', function(html){
			...
			// copy the js files
			fse.copy(path.resolve(self.basePath, 'lib/js'), path.resolve(self.options.output, 'js'), function(error){
				if (!error){
			...
			// build less files
			recess(path.resolve(this.basePath, 'less/bootstrap.less'), {
				compress: true
			}, function(error, obj){
				fs.writeFile(path.resolve(css, 'doctor.css'), obj.output);
			});
		});
